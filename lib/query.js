...
exports.search = function(db, params, conf) {
	...
	rs._read = (size) => {
		...
		if (err || row === undefined) {
		    ...
		    rs.push(JSON.stringify(row_polish(row)) + '\n')
		}
	    })
	}
...
let row_polish = function(row) {
    ['authorslist', 'tagslist'].forEach( key => {
	if (row[key]) row[key] = _.uniq(row[key].split(','))
    });
    ['month', 'day'].forEach( key => {
	if (row[key]) row[key] = exports.pad(row[key])
    });
    return row
}
